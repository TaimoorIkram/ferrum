...
> **IMPORTANT**: _Make sure to copy `.env.example` as `.env` and fill out any missing values. The filled values are recommended specifications so you can directly use them. Follow instructions, if any, in the description of the env variable. Then run `cargo r client`._
